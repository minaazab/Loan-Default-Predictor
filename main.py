...
# ─────────────────────────────────────────────────
# STEP 7: THRESHOLD TUNING ON REAL VAL SET
# ─────────────────────────────────────────────────
print("\n" + "=" * 60)
print("STEP 7: THRESHOLD TUNING (on real validation data)")
print("=" * 60)
...
# Choose threshold: recall >= 60%, highest F1
...
for k in ['Recall','ROC-AUC','Precision','F1']:
    d = r_opt[k] - r_050[k]
    print(f"  {k:<12}: {r_050[k]:.3f} → {r_opt[k]:.3f}  ({'↑' if d>=0 else '↓'}{abs(d):.3f})")
...
# ─────────────────────────────────────────────────
print("\n" + "=" * 60)
print("STEP 10: FAIRNESS ANALYSIS")
print("=" * 60)
...
ax.set_title(f'Confusion Matrix\n✅TP={tp2:,}  ❌FN={fn2:,}  ⚠️FP={fp2:,}  ✅TN={tn2:,}', fontweight='bold')
